...
class Filter(aux._ID):
    ...
    def __init__(self):
        ...
        self._filter_stats = pd.DataFrame({"assay": [], "group": [], "anchor": [], "upper": [], "lower": []})
    ...
    def set_lim(self, lim=None, upper=None, lower=None):
        ...
        """
        if lim is not None:
            self._upper, self._lower = lim, lim
...
{details}
    ...
    def _save_stats(self, assay, group, anchor, upper, lower):
        ...
        """
        new_stats = pd.DataFrame({"assay": [assay], "group": [group], "anchor": [anchor], "upper": [upper], "lower": [lower]})
...
__default_RangeFilter__ = RangeFilter()
__default_IQRFilter__ = IQRFilter()
...
def filter(assay, mode: str = "range", lim: (float or tuple) = None, anchor=None, ignore_nan: bool = True, drop_outliers: bool = False):
    ...
    """
    f = __default_RangeFilter__ if mode == "range" else __default_IQRFilter__
